\begin{algorithm2e}[t]
  ...
  \label{alg:love_kissgp}
    ...
    \If{$\blue \bR$ has not previously been computed}{
      ...
      $\blue \bR$ $\leftarrow$ \triangularsolve{ $\color{blue} \mvmkux{ \bQ }, \bL_\bT$ }
      \tcp*{$\bR = \bL_\bT^{-1} \bQ^\top \bW^\top_\bX \bK_{\bZ\bZ}$  }
    }
    \tcp{$\bR \in \reals^{J \times M}$, $J \ll M$.}
    $\bu$ $\leftarrow$ \sparsemm{ $\blue \bR$, $\bw_\bxtest$ } \\
    $\bu'$ $\leftarrow$ \sparsemm{ $\blue \bR$, $\bw_\bxtestprime$ } \\
