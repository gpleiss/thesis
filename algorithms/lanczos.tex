\begin{algorithm2e}[t!]
  ...
  \label{alg:lanczos}
    ...
    \For{$j \gets 2$ \KwTo J}{
      ...
      $\bq^{(j)}$ $\gets$ $\bv / s_{j-1}$
      \\
      $\bv$ $\gets$ \mvmkxx{ $\bq^{(j)}$ } $ - s_{j-1} \bq_{j-1} $
      \\
      $d_j$ $\gets$ $\bq^{(j)^\top} \bv$
      \\
      $\bv$ $\gets$ $\bv - d_j \bq^{(j)}$
    ...
    \tcp{Collect orthonormal columns $\bq^{(j)}$ and tridiagonal entries $d_j$, $s_j$}
    $\bQ_J$ $\gets$ $\begin{bmatrix} \bq^{(1)},& \ldots,& \bq^{(J)} \end{bmatrix}$
    \\
    $\forall j \in [1, J]$ \: $T_J^{(j,j)}$ $\gets$ $d_j$
    \\
    $\forall j \in [1, J-1]$ \: $T_J^{(j,j-1)}$ $\gets$ $s_j$
    \\
    $\forall j \in [1, J-1]$ \: $T_J^{(j-1,j)}$ $\gets$ $s_j$
    \BlankLine
    \Return{$\bQ_J$, $\bT_J$}
\end{algorithm2e}
