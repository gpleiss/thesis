\begin{algorithm2e}[t!]
  ...
  \SetKwFunction{row}{row\_$\bA$}
  \caption{Partial (Rank-$R$) Pivoted Cholesky Decomposition.}
  \label{alg:piv_chol}
    \Input{$\bd_\bA$ -- diagonal of $\bA$.}
    \NextInput{\row{$i$} -- function for computing the $i^\text{th}$ of matrix $\bA$.}
    ...
    \BlankLine
    \For{$i \gets 1$ \KwTo $R$}{
      \tcp{Find index of largest entry in permutted $\bd_\bA$.}
      $m$ $\gets$ $\argmax_{j \in [i, N]} \bar \bd_\bA^{\left( \pi(j) \right)}$
      \\
      ...
      $\bl^{(i)}$ $\gets$ $\bzero$
      \\
      $\ell^{\left(i, \pi(i) \right)}$ $\gets$ $\sqrt{ \bd_\bA^{\left( \pi(i) \right)} }$
      \\
