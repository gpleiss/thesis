...
\begin{theorem}[Convergence of pivoted Cholesky-preconditioned CG]
  ...
  Assume we are using preconditioned CG to solve the system $\trainK^{-1} \by = (\bK_{\bX\bX} + \sigma_\text{obs}^2 \bI)^{-1} \by$ with preconditioner $\trainP = (\bar\bL_k \bar\bL_k^\top + \sigma_\text{obs}^2 \bI)$.
