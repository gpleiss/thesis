...
\begin{observation}[Properties of the rank-$R$ pivoted Cholesky preconditioner]
  ...
  \begin{enumerate}
    ...
    \item Linear solves with $\trainP_R = \bar\bL_{R} \bar\bL_{R}^{\top} + \sigma_\text{obs}^{2} \bI$ can be performed in $\bigo{NR^{2}}$ time using the Woodbury matrix formula using the reparameterization trick \cite{kingma2014auto}.\footnote{
