...
\autoref{fig:timing_results} shows the speedup obtained by GPU-accelerated BBMM over the leading CPU-based inference engines (Cholesky for Exact/SGPR, \citet{dong2017scalable} for SKI).
...
In \autoref{fig:error_results} we report test mean average error (MAE) for Exact and SGPR models.\footnote{
  ...
  This is because the BBMM inference engine and the inference engine of \citet{dong2017scalable} return identical outputs (see \autoref{app:mod_cg}) even though BBMM is faster.
}
