...
The mean is given by \autoref{eqn:predictive_mean} and the covariance between the two points is given as
...
For example, $\btheta$ might include the lengthscale $\ell$ and outputscale $o^2$ of the kernel function (see \autoref{sec:common_kernels}) as well as the observational noise parameter $\sigma_\text{obs}$.
...
We can use \autoref{eqn:log_lik} to learn the parameters $\btheta$ via gradient-based optimization or sampling.
...
As we will discuss in detail in the next chapters, computing the marginal log likelihood (\autoref{eqn:log_lik}) and the predictive distribution (\autoref{eqn:predictive_mean,eqn:predictive_var}) has been computationally prohibitive for large datasets.
