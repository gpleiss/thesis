...
In the next section we will discuss a fourth variant---Structured Kernel Interpolation \cite{wilson2015kernel}---which is a notably unique method that utilizes inducing points in a different way.
...
\begin{itemize}
    ...
    \[ \loglik(\by \mid \bX, \theta) = -\log p(\by \mid \bX, \theta) + \sum_{i=1}^N \left( k(\bx_i, \bx_i) - \bk_{\bZ\bx_i}^\top \bK_{\bZ\bZ}^{-1} \bk_{\bZ\bx_i} \right). \]
  The diagonal correction term can be derived by applying variational inference to the function values at inducing point locations.
\end{itemize}

\subsection{Structured Kernel Interpolation (SKI)}
