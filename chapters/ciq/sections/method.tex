...
It is worth noting that this procedure requires estimates of $\bK$'s extreme eigenvalues $\lambda_\text{min}$ and $\lambda_\text{max}$.
...
%MINRES \gp{CITE} is an iterative approach to solving $\bK^{-1} \bb$ that is similar to the method of conjugate gradients.
%At each iteration $j$, MINRES produces the best approximate solve $\bd_j \approx \bK^{-1} \bb$ within the \emph{Krylov subspace} of $\bK$ and $\bb$:
%%
%\[ \bd_j = \min_{\bd \in \mathcal{K}_j (\bK, \bb)} \Vert \bK \bd - \bb \Vert_2^2, \]
%%
%where $\mathcal{K}_j (\bK, \bb)$ is the $j^\text{th}$ Krylov subspace of $\bK$ and $\bb$:
%%
%\begin{equation}
  %\mathcal{K}_j (\bK, \bb) \triangleq
  %\text{span} \left\{
    %\bb, \:\: \bK \bb, \:\: \bK^2 \bb, \:\: \ldots, \:\: \bK^{j-1} \bb
  %\right\}.
%\end{equation}
%%
%(Note that each vector in the subspace can be formed simply by multiplying its preceding vector by $\bK$.)
...
%At each iteration $j+1$, we perform the matrix-vector multiplication $\bK \left( \bK^{j-1} \bb \right)$ to form the next Krylov subspace:
%%
%\[ \mathcal{K}_{j+1} (\bK, \bb) = \text{span} \left\{ \mathcal{K}_{j} (\bK, \bb) \:\: \cup \:\: \{ \bK \left( \bK^{j-1} \bb \right) \} \right\}.  \]
%%
%In practice, we can produce each $\bd_j$ through a three-vector recurrence relationship with only a single matrix-vector multiplication at each time step.
...
%If we run $J$ iterations of the modified MINRES, the total computation time of $\bK_{\bZ\bZ}^{-\frac 1 2} \bk_{\bZ\bxtest}$ is $\bigo{J M^2}$.
