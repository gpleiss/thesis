...
This perspective of CG leads to its famous (though arguably loose) convergence bound:
%
...
The most effective preconditioners have simple-to-compute inverses (e.g. diagonal matrices, low rank matrices, etc.), yet closely approximate $\bA^{-1}$.
For example, a common (though often ineffective) preconditioner is the \emph{Jacobi preconditioner}, which is simply the diagonal of $\bA$ (i.e. $\bP = \text{diag}(\bA)$.

The design of effective preconditioners is an active area of research, and far too extensive to adequately review here.
We refer the reader to \cite{saad2003iterative} who devotes two chapters to different preconditioning methods.
...
In practice, there is a far more efficient algorithm---linear conjugate gradients---which arrives at the same solution through a more numerically stable procedure.
We will first introduce this algorithm, and then compare and contrast the two

In fact, one can derive the conjugate gradients algorithm from \cref{eqn:lanczos_solves}, where the $\bu$, $\br$, and $\bd$ vectors
\gp{Finish and fix.}
