...
\subsection{Variational Inference for Non-Conjugate/Large-Scale GPs.}
...
The parameters $\bm$ and $\bS$ (as well as the inducing locations $\bZ$ and associated kernel/likelihood hyperparameters) are chosen to minimize the KL-divergence between the approximate posterior $q(\bf(\cdot), \bu)$ and the true posterior $p(\bf(\cdot), \bu \mid \bX, \by)$.
...
\cref{eqn:elbo} is typically optimized using gradient descent or natural gradient descent methods \cite{hensman2012fast,salimbeni2017doubly}.
